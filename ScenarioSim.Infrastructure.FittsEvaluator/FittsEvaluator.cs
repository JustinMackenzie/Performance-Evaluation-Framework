...
namespace ScenarioSim.Infrastructure.FittsEvaluator
{
    ...
    {
        private ISimulationResultRepository resultRepository;
        ...
        public FittsEvaluator(ISimulationResultRepository resultRepository)
        {
            this.resultRepository = resultRepository;
        }

        public void Evaluate(SimulationResult result)
        {
            Scenario scenario = result.Scenario;

            IEnumerable<SimulationResult> results = resultRepository.GetAllResults(scenario);

            taskDifficultySpeedCollection = new Dictionary<string, List<SpeedDifficultyPair>>();

            foreach (SimulationResult simulationResult in results)
